      ...
      <form class="flex flex-col items-start gap-6 border border-gray-200 p-6" method="GET">
        <div class="flex flex-wrap">
          ...
          @foreach(get_terms('project_category') as $category)
            <label for="{{ $category->slug }}" class="text-gray-600 italic flex items-center text-lg mr-6">
              <input type="checkbox" name="cat[]" id="{{ $category->slug }}" value="{{ $category->slug }}" class="-mt-0.5 mr-1" {{ (!empty($active_categories) && in_array($category->slug, $active_categories) ? 'checked' : '')}}>
              {{ $category->name}}
            </label>
          @endforeach
        </div>
