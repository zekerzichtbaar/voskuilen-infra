<section class="relative {{ $pt }} {{ $pb }}">
    <div class="container">
        <div class="flex gap-24 {{ $links ? "flex-col md:flex-row" : "flex-col" }}">
            ...
            @else
                <div class="flex flex-col lg:flex-row justify-center items-center gap-12 lg:gap-4 {{ $layout == "text-image" ? "order-2" : "order-1" }} {{ $links ? "w-full" : "" }}">
                    @if($one_column_image)
                        <div class="relative aspect-video bg-primary {{ $links ? "h-[50rem] w-full md:w-[40rem] lg:w-[30rem]" : "h-full w-full" }}">
