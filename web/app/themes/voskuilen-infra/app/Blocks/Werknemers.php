...
class Werknemers extends Block
{
    public $name = 'Werknemers';
    public $description = 'A simple Werknemers block.';
    ...
    {
        $werknemer_query_args = ['post_type' => 'werknemer', 'orderby' => 'date', 'order' => 'DESC'];
        $werknemers = new \WP_Query($werknemer_query_args);

        return [
            'title' => get_field('title'),
            'heading' => get_field('heading'),
            'background' => get_field('background'),
            'werknemers' => $werknemers->posts,
        ];
    }

    public function fields()
    {
        $acfFields = new FieldsBuilder('werknemers');
